...
function LanguageSelector({hasAccess}: {hasAccess: boolean}) {
  ...
  const handleLanguageSelect = (langId:string) => {
    if(!hasAccess && !["javascript", "python", "cpp"].includes(langId)) return
    setLanguage(langId);
    setisOpen(false);
  }
  ...
  return (
    <div className='relative' ref={dropdownRef}>
      ...
      rounded-lg transition-all 
       ...
       ${!hasAccess && !["javascript", "python", "cpp"].includes(language) ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        ...
        <ChevronDownIcon className={`size-4 text-gray-400 transition-all duration-300 group-hover:text-gray-300
            ${isOpen ? "rotate-180" : ""}`} />
      </motion.button>
          ...
          >
            ...
            <div className="max-h-[280px] overflow-y-auto overflow-x-hidden">
              {Object.values(LANGUAGE_CONFIG).map((lang, idx) => {
                const isLocked = !hasAccess && !["javascript", "python", "cpp"].includes(lang.id);
